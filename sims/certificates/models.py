...
class Certificate(models.Model):
    ...
    title = models.CharField(
        max_length=300,
        help_text="Full title of the certificate"
    )
    ...
    cme_points_earned = models.PositiveIntegerField(
        default=0,
        help_text="CME points earned for this certificate"
    )
    
    cpd_credits_earned = models.PositiveIntegerField(
        default=0,
        help_text="CPD credits earned for this certificate"
    )
    ...
    def clean(self):
        ...
        if self.certificate_file:
            ...
            if file_extension not in allowed_extensions:
                errors['certificate_file'] = f"File type {file_extension} not allowed. Use: {', '.join(allowed_extensions)}"
    ...
    def save(self, *args, **kwargs):
        ...
        if (self.expiry_date and 
            self.expiry_date <= timezone.now().date() and 
            self.status == 'approved'):
            self.status = 'expired'
        
        # Set automatic expiry date based on certificate type
        if (not self.expiry_date and 
            self.certificate_type and 
            self.certificate_type.validity_period_months):
...
class CertificateReview(models.Model):
    ...
    required_changes = models.TextField(
        blank=True,
        help_text="Specific changes required before approval"
    )
...
class CertificateStatistics(models.Model):
    ...
    def update_statistics(self):
        ...
        if required_types > 0:
            pg_required_certs = certificates.filter(
                certificate_type__is_required=True,
                status='approved'
