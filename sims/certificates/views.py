...
from .forms import (
    CertificateCreateForm, CertificateUpdateForm, CertificateReviewForm,
    CertificateSearchForm, CertificateFilterForm, BulkCertificateApprovalForm
)
...
class CertificateDetailView(LoginRequiredMixin, CertificateAccessMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Add reviews
        context['reviews'] = certificate.reviews.select_related(
            'reviewer'
        ).order_by('-created_at')
        ...
        if certificate.pg:
            context['related_certificates'] = Certificate.objects.filter(
                pg=certificate.pg
            ).exclude(pk=certificate.pk).order_by('-issue_date')[:5]
    ...
    def can_user_review(self, certificate):
        ...
        if user.role == 'admin':
            ...
            return (user == certificate.pg.supervisor and 
                   certificate.status in ['pending', 'under_review'])
...
class CertificateUpdateView(LoginRequiredMixin, CertificateAccessMixin, UpdateView):
    ...
    def form_valid(self, form):
        ...
        messages.success(
            self.request,
            f"Certificate '{form.instance.title}' updated successfully"
        )
...
class CertificateDeleteView(LoginRequiredMixin, CertificateAccessMixin, DeleteView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        messages.success(
            request,
            f"Certificate '{certificate.title}' has been deleted"
        )
...
class CertificateReviewCreateView(LoginRequiredMixin, CertificateAccessMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        messages.success(
            self.request,
            f"Review submitted for certificate '{self.certificate.title}'"
        )
...
class CertificateDashboardView(LoginRequiredMixin, CertificateAccessMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['expiring_certificates'] = certificates.filter(
            expiry_date__isnull=False,
            expiry_date__lte=today + timedelta(days=60),
            expiry_date__gt=today,
            status='approved'
        ).select_related('pg', 'certificate_type').order_by('expiry_date')[:5]
        ...
        if user.role == 'pg':
            ...
            user_required_certs = certificates.filter(
                certificate_type__in=required_types,
                status='approved'
            ).values('certificate_type').distinct().count()
            
            context['compliance_rate'] = (
                (user_required_certs / required_types.count() * 100) 
                if required_types.count() > 0 else 100
            )
    ...
    def get_performance_metrics(self, certificates):
        ...
        return {
            ...
            'top_issuers': certificates.values('issuing_organization').annotate(
                count=Count('id')
            ).order_by('-count')[:5],
        }
    ...
    def calculate_average_review_time(self, certificates):
        ...
        approved_certs = certificates.filter(
            status='approved',
        ...
        try:
            for cert_id in form.cleaned_data['certificates']:
                ...
                if (self.request.user.role == 'supervisor' and 
                    certificate.pg.supervisor != self.request.user):
                    continue
            ...
            if approved_count > 0:
                messages.success(
                    self.request,
                    f"Successfully approved {approved_count} certificates"
                )
            if rejected_count > 0:
                messages.success(
                    self.request,
                    f"Successfully rejected {rejected_count} certificates"
                )
            
        except Exception as e:
            messages.error(
                self.request,
                f"Error processing certificates: {str(e)}"
            )
...
def export_certificates_csv(request):
    ...
    # Write header
    writer.writerow([
        'PG Name', 'PG Username', 'Certificate Title', 'Certificate Type',
        'Issuing Organization', 'Issue Date', 'Expiry Date', 'Status',
        'Is Verified', 'CME Points', 'CPD Credits', 'Created Date'
    ])
...
class CertificateComplianceView(LoginRequiredMixin, CertificateAccessMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        compliance_data = []
        for pg in pgs:
            pg_certs = Certificate.objects.filter(
                pg=pg, 
                status='approved'
            )
            ...
            missing_required = required_types.exclude(
                id__in=pg_cert_types
            )
            
            compliance_rate = (
                (len(pg_cert_types & required_type_ids) / len(required_type_ids) * 100)
                if required_type_ids else 100
            )
        ...
        context['overall_compliance'] = (
            sum(data['compliance_rate'] for data in compliance_data) / 
            len(compliance_data) if compliance_data else 100
        )
