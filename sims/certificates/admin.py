...
class CertificateResource(resources.ModelResource):
    ...
    class Meta:
        model = Certificate
        fields = ('id', 'pg__username', 'certificate_type__name', 'title', 
...
class CertificateTypeAdmin(admin.ModelAdmin):
    ...
    list_display = ('name', 'category', 'is_required', 'validity_period_months', 
                   'certificate_count', 'is_active', 'created_at')
...
class CertificateAdmin(ImportExportModelAdmin):
    ...
    list_display = ('title', 'get_pg_name', 'certificate_type', 'issuing_organization', 
                   'issue_date', 'expiry_date', 'status_badge', 'review_status', 
                   'created_at')
    list_filter = ('status', 'certificate_type', 'issue_date', 'expiry_date', 
                  'created_at', 'issuing_organization')
    search_fields = ('title', 'pg__username', 'pg__first_name', 'pg__last_name',
                    'issuing_organization', 'certificate_number')
    ...
    fieldsets = (
        ...
        ('Documentation', {
            'fields': ('certificate_file', 'additional_documents'),
            'classes': ('collapse',)
        }),
        ('Verification', {
...
class CertificateReviewAdmin(admin.ModelAdmin):
    ...
    search_fields = ('certificate__title', 'certificate__pg__username', 
                    'certificate__pg__first_name', 'certificate__pg__last_name',
                    'reviewer__username', 'comments')
    ordering = ('-created_at',)
    
    fieldsets = (
        ('Review Details', {
            'fields': ('certificate', 'reviewer', 'status', 'review_date')
        }),
        ('Feedback', {
            'fields': ('comments', 'recommendations', 'required_changes')
        }),
        ('Audit Information', {
    ...
    def get_queryset(self, request):
        """Filter reviews based on user role"""
        qs = super().get_queryset(request).select_related(
            'certificate__pg', 'certificate__certificate_type', 'reviewer'
        )
        ...
        if request.user.is_superuser or request.user.role == 'admin':
            ...
            return qs.filter(
                Q(certificate__pg__supervisor=request.user) | 
    ...
    def certificate_display(self, obj):
        ...
        if obj.certificate:
            ...
            return format_html(
                '<a href="{}" title="View Certificate">{}</a>',
                url, obj.certificate.title
            )
    ...
    def formfield_for_foreignkey(self, db_field, request, **kwargs):
        ...
        if db_field.name == "certificate":
            if request.user.role == 'supervisor':
                kwargs["queryset"] = Certificate.objects.filter(
...
def check_expiring_certificates(modeladmin, request, queryset):
    ...
    messages.info(
        request, 
        f"Found {expiring_soon} certificates expiring within 30 days"
    )
...
class CertificateAdminConfig:
    ...
    class Media:
        css = {
            'all': ('admin/css/certificate_admin.css',)
        }
...
class CertificateReportAdmin:
    ...
    def expiring_certificates_report(self, request):
        ...
        return TemplateResponse(
            request, 
            'admin/certificates/expiring_report.html', 
            context
        )
    ...
    def compliance_report(self, request):
        ...
        return TemplateResponse(
            request, 
            'admin/certificates/compliance_report.html', 
            context
        )
