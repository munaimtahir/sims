...
from .forms import (
    RotationCreateForm, RotationUpdateForm, RotationEvaluationForm,
    RotationSearchForm, BulkRotationAssignmentForm, RotationFilterForm
)
    ...
    def get_queryset(self):
        ...
        if self.request.user.role == 'supervisor':
            queryset = queryset.filter(
                ...
                Q(pg__supervisor=self.request.user)
            )
...
class RotationDetailView(LoginRequiredMixin, RotationAccessMixin, DetailView):
    ...
    def get_object(self):
        ...
        if self.request.user.role == 'supervisor':
            if (rotation.supervisor != self.request.user and 
                rotation.pg.supervisor != self.request.user):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Add evaluations
        context['evaluations'] = rotation.evaluations.select_related(
            'evaluator'
        ).order_by('-created_at')
        ...
        if evaluations.exists():
            context['average_score'] = evaluations.aggregate(
                Avg('score')
            )['score__avg']
        ...
        if rotation.pg:
            context['related_rotations'] = Rotation.objects.filter(
                pg=rotation.pg
            ).exclude(pk=rotation.pk).order_by('-start_date')[:5]
    ...
    def can_user_evaluate(self, rotation):
        ...
        if user.role == 'admin':
            ...
            return (user == rotation.supervisor or 
                   user == rotation.pg.supervisor)
...
class RotationCreateView(LoginRequiredMixin, RotationAccessMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        messages.success(
            self.request,
            f"Rotation created successfully for {form.instance.pg.get_full_name()}"
        )
...
class RotationUpdateView(LoginRequiredMixin, RotationAccessMixin, UpdateView):
    ...
    def form_valid(self, form):
        ...
        messages.success(
            self.request,
            f"Rotation updated successfully"
        )
...
class RotationDeleteView(LoginRequiredMixin, RotationAccessMixin, DeleteView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        messages.success(
            request,
            f"Rotation for {rotation.pg.get_full_name()} has been cancelled"
        )
...
class RotationEvaluationCreateView(LoginRequiredMixin, RotationAccessMixin, CreateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if request.user.role == 'supervisor':
            if (self.rotation.supervisor != request.user and 
                self.rotation.pg.supervisor != request.user):
...
class RotationEvaluationDetailView(LoginRequiredMixin, RotationAccessMixin, DetailView):
    ...
    def get_object(self):
        ...
        if user.role == 'supervisor':
            if (rotation.supervisor != user and 
                rotation.pg.supervisor != user and
...
class RotationDashboardView(LoginRequiredMixin, RotationAccessMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if user.role == 'admin':
            ...
            rotations = Rotation.objects.filter(
                Q(supervisor=user) | Q(pg__supervisor=user)
            )
        elif user.role == 'pg':
        ...
        context['stats'] = {
            ...
            'upcoming_rotations': rotations.filter(
                status='planned',
        ...
        # Current rotations
        context['current_rotations'] = rotations.filter(
            status='ongoing'
        ).select_related('pg', 'department', 'hospital', 'supervisor')[:5]
        
        # Upcoming rotations
        context['upcoming_rotations'] = rotations.filter(
            status='planned',
            start_date__gt=today
        ).select_related('pg', 'department', 'hospital', 'supervisor').order_by('start_date')[:5]
        
        # Recent evaluations (for supervisors and admins)
        if user.role in ['admin', 'supervisor']:
            context['recent_evaluations'] = RotationEvaluation.objects.filter(
                rotation__in=rotations
            ).select_related(
                'rotation__pg', 'evaluator'
    ...
    def get_performance_metrics(self, rotations):
        ...
        evaluations = RotationEvaluation.objects.filter(
            rotation__in=rotations,
            score__isnull=False
        )
        ...
        return {
            ...
            'passing_rate': (
                evaluations.filter(score__gte=60).count() / 
                evaluations.count() * 100
            ),
            'excellence_rate': (
                evaluations.filter(score__gte=90).count() / 
                evaluations.count() * 100
            ),
        }
...
class BulkRotationAssignmentView(LoginRequiredMixin, RotationAccessMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        try:
            ...
            messages.success(
                self.request,
                f"Successfully created {created_count} rotations"
            )
            
        except Exception as e:
            messages.error(
                self.request,
                f"Error creating rotations: {str(e)}"
            )
...
def rotation_stats_api(request):
    ...
    stats = {
        ...
        'by_department': list(
            rotations.values('department__name')
...
def export_rotations_csv(request):
    ...
    # Write header
    writer.writerow([
        'PG Name', 'PG Username', 'Department', 'Hospital', 
        'Supervisor', 'Start Date', 'End Date', 'Status',
        'Duration (Days)', 'Created Date'
    ])
...
def rotation_quick_stats(request):
    ...
    stats = {
        'ongoing': rotations.filter(status='ongoing').count(),
        'upcoming': rotations.filter(
            status='planned',
            start_date__gt=today
        ).count(),
        'overdue': rotations.filter(
            end_date__lt=today,
            status='ongoing'
        ).count(),
