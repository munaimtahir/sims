...
class Hospital(models.Model):
    ...
    name = models.CharField(
        max_length=200,
        help_text="Official name of the hospital"
    )
    ...
    address = models.TextField(
        blank=True,
        help_text="Complete hospital address"
    )
    ...
    facilities = models.TextField(
        blank=True,
        help_text="Available facilities and equipment"
    )
...
class Rotation(models.Model):
    ...
    start_date = models.DateField(
        help_text="Start date of the rotation"
    )
    
    end_date = models.DateField(
        help_text="End date of the rotation"
    )
        ...
        if self.start_date and self.end_date:
            ...
            if self.pg:
                ...
                for rotation in overlapping:
                    if (self.start_date <= rotation.end_date and 
                        self.end_date >= rotation.start_date):
                        errors['start_date'] = f"This rotation overlaps with existing rotation: {rotation}"
                        break
...
class RotationEvaluation(models.Model):
    ...
    score = models.IntegerField(
        null=True,
        blank=True,
        help_text="Numerical score (0-100)"
    )
    
    comments = models.TextField(
        blank=True,
        help_text="Detailed comments and feedback"
    )
