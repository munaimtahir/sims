...
class RotationCreateForm(forms.ModelForm):
    ...
    class Meta:
        ...
        fields = [
            ...
            'start_date', 'end_date', 'objectives', 
            'learning_outcomes', 'requirements', 'status'
        ]
        widgets = {
    ...
    def clean(self):
        ...
        if pg and start_date and end_date:
            overlapping_rotations = Rotation.objects.filter(
                pg=pg,
                status__in=['planned', 'ongoing', 'pending']
            ).exclude(pk=self.instance.pk if self.instance else None)
            
            for rotation in overlapping_rotations:
                if (start_date <= rotation.end_date and 
                    end_date >= rotation.start_date):
        ...
        if department and hospital:
            if department.hospital != hospital:
                raise ValidationError("Selected department does not belong to the selected hospital")
...
class RotationUpdateForm(RotationCreateForm):
    ...
    def _apply_status_restrictions(self):
        ...
        # Completed rotations: very limited editing
            ...
            readonly_fields = ['pg', 'start_date', 'end_date', 'department', 'hospital', 'supervisor']
            for field_name in readonly_fields:
                if field_name in self.fields:
                    self.fields[field_name].disabled = True
                    self.fields[field_name].help_text += " (Cannot be changed for completed rotation)"
...
class RotationEvaluationForm(forms.ModelForm):
    ...
    def _setup_evaluation_type_choices(self):
        ...
        # Filter choices
        filtered_choices = [
            choice for choice in RotationEvaluation.EVALUATION_TYPES
        ...
        if self.rotation:
            ...
            filtered_choices = [
                choice for choice in filtered_choices
                if choice[0] not in existing_evaluations
            ]
    ...
    def _setup_field_requirements(self):
        ...
        self.fields['comments'].help_text = "Detailed feedback on performance, achievements, and areas for improvement"
    ...
    def clean(self):
        ...
        if score is not None and score < 60:
            if not comments or len(comments.strip()) < 50:
                raise ValidationError(
                    "Detailed comments are required for scores below 60"
                )
