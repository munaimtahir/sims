...
class RotationResource(resources.ModelResource):
    ...
    class Meta:
        model = Rotation
        fields = ('id', 'pg__username', 'department__name', 'hospital__name',
                 'start_date', 'end_date', 'supervisor__username', 'status',
...
class DepartmentAdmin(admin.ModelAdmin):
    ...
    def rotation_count(self, obj):
        ...
        if count > 0:
            ...
            return format_html(
                '<a href="{}?department__id__exact={}">{} rotations</a>',
                url, obj.id, count
            )
        return "0 rotations"
...
class RotationAdmin(ImportExportModelAdmin):
    ...
    list_display = ('get_pg_name', 'department', 'hospital', 'start_date', 
                   'end_date', 'duration_display', 'status_badge', 'supervisor', 
                   'evaluation_count', 'created_at')
    list_filter = ('status', 'department', 'hospital', 'start_date', 'end_date', 
                  'created_at', 'supervisor')
    search_fields = ('pg__username', 'pg__first_name', 'pg__last_name',
                    'department__name', 'hospital__name', 'supervisor__username')
    date_hierarchy = 'start_date'
    ordering = ('-start_date', 'pg__last_name')
    
    fieldsets = (
        ('Basic Information', {
            'fields': ('pg', 'department', 'hospital', 'supervisor')
        }),
        ('Rotation Period', {
        ...
        ('Completion & Feedback', {
            'fields': ('completion_certificate', 'feedback', 'notes'),
            'classes': ('collapse',)
        }),
        ('Audit Information', {
    ...
    def get_queryset(self, request):
        ...
        qs = super().get_queryset(request).select_related(
            'pg', 'department', 'hospital', 'supervisor', 'created_by'
...
class RotationEvaluationAdmin(admin.ModelAdmin):
    ...
    list_display = ('rotation_display', 'evaluator', 'evaluation_type', 
                   'score_display', 'status', 'created_at')
    list_filter = ('evaluation_type', 'status', 'score', 'created_at',
                  'rotation__department', 'rotation__hospital')
    search_fields = ('rotation__pg__username', 'rotation__pg__first_name',
                    'rotation__pg__last_name', 'evaluator__username',
                    'comments')
    ordering = ('-created_at',)
    
    fieldsets = (
        ('Evaluation Details', {
            'fields': ('rotation', 'evaluator', 'evaluation_type')
        }),
        ('Assessment', {
            'fields': ('score', 'status', 'comments', 'recommendations')
        }),
        ('Audit Information', {
    ...
    def rotation_display(self, obj):
        ...
        if obj.rotation:
            ...
            return format_html(
                ...
                url, obj.rotation.pg.get_full_name() if obj.rotation.pg else 'No PG',
...
class RotationAdminConfig:
    ...
    class Media:
        css = {
            'all': ('admin/css/rotation_admin.css',)
        }
