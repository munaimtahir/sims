...
class CaseCategory(models.Model):
    ...
    description = models.TextField(
        blank=True,
        help_text="Description of the case category"
    )
...
class ClinicalCase(models.Model):
    ...
    case_title = models.CharField(
        max_length=300,
        help_text="Descriptive title for the case"
    )
    ...
    date_encountered = models.DateField(
        help_text="Date when the case was encountered"
    )
    ...
    family_history = models.TextField(
        blank=True,
        help_text="Relevant family history"
    )
    ...
    differential_diagnosis = models.TextField(
        blank=True,
        help_text="Differential diagnosis considerations"
    )
    
    management_plan = models.TextField(
        help_text="Management and treatment plan"
    )
    ...
    learning_objectives = models.TextField(
        blank=True,
        help_text="Learning objectives for this case"
    )
    ...
    literature_review = models.TextField(
        blank=True,
        help_text="Relevant literature review and evidence"
    )
    
    # Follow-up and outcomes
    outcome = models.TextField(
        blank=True,
        help_text="Patient outcome and follow-up"
    )
    
    follow_up_plan = models.TextField(
        blank=True,
        help_text="Follow-up plan and actions required"
    )
    ...
    is_active = models.BooleanField(
        default=True,
        help_text="Whether this case is active"
    )
    ...
    def clean(self):
        ...
        if (self.date_encountered and 
            self.date_encountered < (timezone.now().date() - timedelta(days=730))):
...
class CaseReview(models.Model):
    ...
    clinical_reasoning_feedback = models.TextField(
        blank=True,
        help_text="Feedback on clinical reasoning"
    )
    
    documentation_feedback = models.TextField(
        blank=True,
        help_text="Feedback on case documentation"
    )
    
    learning_points_feedback = models.TextField(
        blank=True,
        help_text="Feedback on learning points identified"
    )
...
class CaseStatistics(models.Model):
    ...
    total_cases = models.PositiveIntegerField(
        default=0,
        help_text="Total number of cases submitted"
    )
    
    approved_cases = models.PositiveIntegerField(
        default=0,
        help_text="Number of approved cases"
    )
    
    pending_cases = models.PositiveIntegerField(
        default=0,
        help_text="Number of cases pending review"
    )
    
    draft_cases = models.PositiveIntegerField(
        default=0,
        help_text="Number of draft cases"
    )
    
    # Case complexity distribution
    simple_cases = models.PositiveIntegerField(
        default=0,
        help_text="Number of simple complexity cases"
    )
    
    moderate_cases = models.PositiveIntegerField(
        default=0,
        help_text="Number of moderate complexity cases"
    )
    
    complex_cases = models.PositiveIntegerField(
        default=0,
        help_text="Number of complex cases"
    )
    
    highly_complex_cases = models.PositiveIntegerField(
        default=0,
        help_text="Number of highly complex cases"
    )
    
    # Performance metrics
    average_self_score = models.FloatField(
        default=0.0,
        help_text="Average self-assessment score"
    )
    
    average_supervisor_score = models.FloatField(
        default=0.0,
        help_text="Average supervisor assessment score"
    )
    
    completion_rate = models.FloatField(
        default=0.0,
        help_text="Percentage of cases completed vs required"
    )
    
    # Time metrics
    average_submission_time = models.FloatField(
        default=0.0,
        help_text="Average days from encounter to submission"
    )
    
    overdue_cases = models.PositiveIntegerField(
        default=0,
        help_text="Number of overdue case submissions"
    )
    ...
    def update_statistics(self):
        ...
        self_scores = cases.filter(
            self_assessment_score__isnull=False
        ).values_list('self_assessment_score', flat=True)
