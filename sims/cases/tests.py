...
class CaseCategoryModelTest(TestCase):
    ...
    def setUp(self):
        self.category = CaseCategory.objects.create(
            name="Cardiology",
            description="Heart and cardiovascular cases",
            color_code="#FF5722"
        )
...
class CaseViewsTest(TestCase):
    ...
    def test_case_update_view(self):
        ...
        response = self.client.post(reverse('cases:case_update', kwargs={'pk': self.case.pk}), data=form_data)
...
class CaseIntegrationTest(TestCase):
    ...
    def test_complete_case_workflow(self):
        ...
        response = self.client.post(
            reverse('cases:case_review', kwargs={'case_pk': case.pk}),
            data=review_data
        )
        self.assertEqual(response.status_code, 302)
