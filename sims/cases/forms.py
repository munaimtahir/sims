...
class ClinicalCaseForm(forms.ModelForm):
    ...
    class Meta:
        model = ClinicalCase
        fields = [
            'date_encountered', 'rotation', 'supervisor', 'case_title', 'category',
            'patient_age', 'patient_gender', 'chief_complaint',
            'history_of_present_illness', 'physical_examination',
            'primary_diagnosis', 'differential_diagnosis', 'management_plan',
            'learning_objectives', 'clinical_reasoning', 'learning_points',
            'challenges_faced', 'outcome', 'follow_up_plan',
            'case_files', 'case_images'
        ]
