        ...
        if hasattr(self.request.user, 'role'):
            if self.request.user.role == 'pg':
                ...
                queryset = queryset.filter(
                    ...
                    Q(rotation__supervisor=self.request.user)
                )
...
class CaseDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_queryset(self):
        ...
        queryset = ClinicalCase.objects.select_related(
            ...
            'secondary_diagnoses', 'procedures_performed', 'competencies_achieved'
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        # Get related reviews
        context['reviews'] = CaseReview.objects.filter(
            case=self.object
        ...
        context['can_review'] = (
            ...
            self.request.user.role == 'supervisor' and
            (self.object.supervisor == self.request.user or 
    ...
    def _can_edit_case(self):
        ...
        if hasattr(self.request.user, 'role'):
            if self.request.user.role == 'pg':
                return (self.object.pg == self.request.user and 
                       self.object.status not in ['approved'])
            elif self.request.user.role == 'supervisor':
...
def submit_case(request, pk):
    ...
    # Check permissions
    if not (hasattr(request.user, 'role') and request.user.role == 'pg' and 
            case.pg == request.user):
...
class CaseReviewCreateView(LoginRequiredMixin, RoleRequiredMixin, CreateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if hasattr(request.user, 'role') and request.user.role == 'supervisor':
            if not (self.case.supervisor == request.user or 
                   (self.case.rotation and self.case.rotation.supervisor == request.user)):
...
def case_statistics_view(request):
    ...
    if request.user.role == 'pg':
        ...
        context.update({
            'user_stats': stats,
            'recent_cases': ClinicalCase.objects.filter(
                pg=request.user
