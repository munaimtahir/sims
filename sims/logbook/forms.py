...
class LogbookEntryCreateForm(forms.ModelForm):
    ...
    class Meta:
        model = LogbookEntry
        fields = [
            'pg', 'date', 'rotation', 'supervisor', 'case_title', 'template',
            'patient_age', 'patient_gender', 'patient_chief_complaint', 
            'patient_history_summary', 'primary_diagnosis', 'secondary_diagnoses',
            'procedures', 'skills', 'investigations_ordered', 'clinical_reasoning',
            'learning_points', 'challenges_faced', 'follow_up_required',
        ...
        widgets = {
            ...
            'procedures': forms.SelectMultiple(
    ...
    def _setup_field_querysets(self):
        ...
        if self.user.role == 'admin':
            ...
            ).select_related('department', 'hospital').order_by('department__name')
            ...
            self.fields['rotation'].queryset = Rotation.objects.filter(
                pg=self.user
            ).select_related('department', 'hospital').order_by('start_date')
        
        # Filter other querysets to active only
        self.fields['template'].queryset = LogbookTemplate.objects.filter(
            is_active=True
        ...
        self.fields['procedures'].queryset = Procedure.objects.filter(
            is_active=True
        ).order_by('category', 'difficulty_level', 'name')
    ...
    def _setup_field_requirements(self):
        ...
        required_fields = [
            'date', 'patient_age', 'patient_gender', 'patient_chief_complaint',
            'primary_diagnosis', 'clinical_reasoning', 'learning_points'
        ]
    ...
    def _setup_help_text(self):
        ...
        self.fields['case_title'].help_text = "Brief, descriptive title (will be auto-generated if left blank)"
        ...
        self.fields['patient_history_summary'].help_text = "Relevant medical history, medications, social history"
        self.fields['primary_diagnosis'].help_text = "Primary or working diagnosis"
        self.fields['secondary_diagnoses'].help_text = "Secondary diagnoses or differential diagnoses"
    ...
    def clean(self):
        ...
        if supervisor and pg:
            if (pg.supervisor and supervisor != pg.supervisor and 
                not supervisor.role == 'admin'):
                raise ValidationError({
        ...
        if not cleaned_data.get('case_title'):
            if primary_diagnosis and patient_age is not None:
                ...
                cleaned_data['case_title'] = f"{patient_age}y {gender_display} - {primary_diagnosis.name}"
    ...
    def clean_procedures(self):
        ...
        if procedures:
            ...
            if procedures.count() > 10:
                raise ValidationError(
                    "Please select a maximum of 10 procedures for a single case"
                )
    ...
    def clean_skills(self):
        ...
        if skills:
            ...
            if skills.count() > 15:
                raise ValidationError(
                    "Please select a maximum of 15 skills for a single case"
                )
...
class LogbookReviewForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            'feedback': forms.Textarea(
                attrs={'rows': 8, 'class': 'form-control',
    ...
    def _setup_help_text(self):
        ...
        self.fields['feedback'].help_text = "Comprehensive feedback on clinical performance and learning"
        self.fields['strengths_identified'].help_text = "Specific strengths demonstrated by the trainee"
        self.fields['areas_for_improvement'].help_text = "Areas requiring development or improvement"
        ...
        self.fields['clinical_knowledge_score'].help_text = "Clinical knowledge demonstration (1-10)"
        self.fields['clinical_skills_score'].help_text = "Clinical skills performance (1-10)"
        self.fields['professionalism_score'].help_text = "Professional behavior and communication (1-10)"
    ...
    def clean(self):
        ...
        if all([clinical_knowledge_score, clinical_skills_score, professionalism_score, overall_score]):
            component_avg = (clinical_knowledge_score + clinical_skills_score + professionalism_score) / 3
        ...
        if status == 'approved':
            if not all([clinical_knowledge_score, clinical_skills_score, 
                       professionalism_score, overall_score]):
                raise ValidationError(
                    "All assessment scores are required when approving an entry"
                )
...
class BulkLogbookActionForm(forms.Form):
    ...
    def clean(self):
        ...
        if action == 'assign_supervisor' and not new_supervisor:
            raise ValidationError({
                'new_supervisor': "Please select a supervisor for assignment"
            })
...
class QuickLogbookEntryForm(forms.ModelForm):
    ...
    class Meta:
        model = LogbookEntry
        fields = [
            'date', 'rotation', 'case_title', 'patient_age', 'patient_gender',
            'patient_chief_complaint', 'primary_diagnosis', 'procedures',
            'learning_points'
        ]
        widgets = {
            'date': forms.DateInput(
            ...
            'learning_points': forms.Textarea(
                attrs={'rows': 3, 'class': 'form-control',
                       'placeholder': 'Key learning points...'}
            ),
            'rotation': forms.Select(attrs={'class': 'form-control'}),
        }
...
class LogbookTemplateForm(forms.ModelForm):
    ...
    class Meta:
        ...
        fields = [
            'name', 'template_type', 'description', 'is_default', 'is_active',
            'completion_guidelines', 'example_entries'
        ]
        widgets = {
