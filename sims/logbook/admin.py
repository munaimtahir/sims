...
from .models import (
    LogbookEntry, Procedure, Diagnosis, Skill, 
    LogbookTemplate, LogbookReview, LogbookStatistics
)
...
class LogbookEntryResource(resources.ModelResource):
    ...
    class Meta:
        model = LogbookEntry
        fields = ('id', 'pg__username', 'date', 'rotation__department__name',
                 'patient_age', 'patient_gender', 'primary_diagnosis__name',
                 'procedures_performed', 'skills_demonstrated', 'learning_points',
                 'supervisor__username', 'status', 'created_at')
        export_order = fields
        import_id_fields = ('id',)
...
class ProcedureAdmin(admin.ModelAdmin):
    ...
    list_display = ('name', 'category', 'difficulty_level', 'cme_points', 
    ...
    fieldsets = (
        ...
        ('Prerequisites', {
            'fields': ('required_skills', 'prerequisites'),
            'classes': ('collapse',)
        }),
    )
    ...
    def usage_count(self, obj):
        ...
        if count > 0:
            ...
            return format_html(
                '<a href="{}?procedures__id__exact={}">{} entries</a>',
                url, obj.id, count
            )
        return "0 entries"
...
class DiagnosisAdmin(admin.ModelAdmin):
    ...
    list_display = ('name', 'category', 'icd_code', 'usage_count', 
    ...
    fieldsets = (
        (None, {
            'fields': ('name', 'category', 'icd_code', 'is_active')
        }),
        ('Details', {
...
class SkillAdmin(admin.ModelAdmin):
    ...
    list_display = ('name', 'category', 'level', 'usage_count', 
                   'is_active', 'created_at')
    ...
    fieldsets = (
        (None, {
            'fields': ('name', 'category', 'level', 'is_active')
        }),
        ('Details', {
    ...
    def usage_count(self, obj):
        ...
        if count > 0:
            url = reverse('admin:logbook_logbookentry_changelist')
            return format_html(
                '<a href="{}?skills__id__exact={}">{} entries</a>',
                url, obj.id, count
            )
        return "0 entries"
    usage_count.short_description = "Usage"
...
class LogbookTemplateAdmin(admin.ModelAdmin):
    ...
    list_display = ('name', 'template_type', 'is_default', 'usage_count',
    ...
    fieldsets = (
        ...
        ('Guidelines', {
            'fields': ('completion_guidelines', 'example_entries'),
            'classes': ('collapse',)
        }),
        ('Audit Information', {
            'fields': ('created_by',),
...
class LogbookEntryAdmin(ImportExportModelAdmin):
    ...
    list_display = ('get_entry_title', 'get_pg_name', 'date', 'get_rotation',
                   'primary_diagnosis', 'procedure_count', 'status_badge',
                   'review_status', 'created_at')
    list_filter = ('status', 'date', 'patient_gender', 'created_at',
                  'rotation__department', 'supervisor')
    search_fields = ('pg__username', 'pg__first_name', 'pg__last_name',
                    'case_title', 'patient_chief_complaint', 'learning_points',
    ...
    fieldsets = (
        ...
        ('Patient Information', {
            'fields': ('patient_age', 'patient_gender', 'patient_chief_complaint',
                      'patient_history_summary'),
            'classes': ('collapse',)
        }),
        ('Clinical Details', {
            'fields': ('primary_diagnosis', 'secondary_diagnoses', 'procedures',
                      'skills', 'investigations_ordered'),
            'classes': ('collapse',)
        }),
        ('Learning & Reflection', {
            'fields': ('clinical_reasoning', 'learning_points', 'challenges_faced',
                      'supervisor_feedback', 'follow_up_required'),
            'classes': ('collapse',)
        }),
        ('Assessment & Status', {
            'fields': ('self_assessment_score', 'supervisor_assessment_score',
                      'status', 'template'),
            'classes': ('collapse',)
        }),
        ('Audit Information', {
    ...
    def get_pg_name(self, obj):
        ...
        if obj.pg:
            ...
            return format_html(
                ...
                url, obj.pg.get_full_name() or obj.pg.username
            )
    ...
    def approve_entries(self, request, queryset):
        ...
        for entry in queryset.filter(status='submitted'):
            ...
            if (request.user.role == 'supervisor' and 
                entry.pg.supervisor != request.user):
                continue
...
class LogbookReviewAdmin(admin.ModelAdmin):
    ...
    list_display = ('logbook_entry_display', 'reviewer', 'status', 
                   'review_date', 'created_at')
    list_filter = ('status', 'review_date', 'created_at', 
                  'logbook_entry__rotation__department')
    search_fields = ('logbook_entry__case_title', 'logbook_entry__pg__username',
                    'logbook_entry__pg__first_name', 'logbook_entry__pg__last_name',
                    'reviewer__username', 'feedback')
    ordering = ('-created_at',)
    
    fieldsets = (
        ('Review Details', {
            'fields': ('logbook_entry', 'reviewer', 'status', 'review_date')
        }),
        ('Feedback', {
            'fields': ('feedback', 'strengths_identified', 'areas_for_improvement',
                      'recommendations', 'follow_up_required')
        }),
        ('Assessment', {
            'fields': ('clinical_knowledge_score', 'clinical_skills_score',
    ...
    def get_queryset(self, request):
        """Filter reviews based on user role"""
        qs = super().get_queryset(request).select_related(
            'logbook_entry__pg', 'logbook_entry__rotation__department', 'reviewer'
        )
        ...
        if request.user.is_superuser or request.user.role == 'admin':
            ...
            return qs.filter(
                Q(logbook_entry__pg__supervisor=request.user) | 
    ...
    def formfield_for_foreignkey(self, db_field, request, **kwargs):
        ...
        if db_field.name == "logbook_entry":
            if request.user.role == 'supervisor':
                kwargs["queryset"] = LogbookEntry.objects.filter(
                    pg__supervisor=request.user
                )
            elif request.user.role == 'pg':
...
def check_overdue_entries(modeladmin, request, queryset):
    ...
    overdue_count = LogbookEntry.objects.filter(
    ...
    messages.info(
        request, 
        f"Found {overdue_count} draft entries older than 7 days"
    )
...
class LogbookAdminConfig:
    ...
    class Media:
        css = {
            'all': ('admin/css/logbook_admin.css',)
        }
        js = ('admin/js/logbook_admin.js',)

# Apply custom styling
for admin_class in [LogbookEntryAdmin, LogbookReviewAdmin, ProcedureAdmin, DiagnosisAdmin, SkillAdmin]:
...
class LogbookReportAdmin:
    ...
    def get_urls(self):
        ...
        custom_urls = [
            path('reports/completion/', 
                 ...
                 name='logbook_completion_report'),
            path('reports/procedures/', 
                 self.procedures_report,
                 name='logbook_procedures_report'),
            path('reports/learning-analytics/', 
    ...
    def completion_report(self, request):
        ...
        return TemplateResponse(
            request, 
            'admin/logbook/completion_report.html', 
            context
        )
    ...
    def procedures_report(self, request):
        ...
        procedure_stats = Procedure.objects.annotate(
            usage_count=Count('logbook_entries')
        ...
        return TemplateResponse(
            request, 
            'admin/logbook/procedures_report.html', 
            context
        )
    ...
    def learning_analytics_report(self, request):
        ...
        analytics = {
            ...
            'top_diagnoses': Diagnosis.objects.annotate(
                usage=Count('primary_entries')
            ).order_by('-usage')[:10],
        }
        ...
        return TemplateResponse(
            request, 
            'admin/logbook/learning_analytics_report.html', 
            context
        )
